...
import MetalPrices from "./GoldPriceChart";
...
const DashboardContent = () => {
  ...
  // Calculate totalPurchasePrice and totalSoldProfit as in PortfolioSummary
  const totalPurchasePrice = coins.filter(coin => !coin.is_sold).reduce((sum, coin) => sum + coin.purchase_price, 0);
  const totalSoldProfit = coins.filter(coin => coin.is_sold && coin.sold_price != null).reduce((sum, coin) => sum + (coin.sold_price - coin.purchase_price), 0);
  const calculatedCashReserves = 24000 - totalPurchasePrice + totalSoldProfit;
  const totalMarketValue = coins.filter(coin => !coin.is_sold).reduce((sum, coin) => sum + coin.current_value, 0);
  // Set placeholderTotalValue to (calculatedCashReserves + totalMarketValue) - 24000
  const placeholderTotalValue = (calculatedCashReserves + totalMarketValue) - 24000;
        ...
        <TabsContent value="performance" className="space-y-6">
          <Card className="p-6 bg-card">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <MetalPrices />
              <UICard className="h-40 flex flex-col items-center justify-center">
                <div className="text-lg font-semibold mb-2">Total Profit</div>
                <div className="text-3xl font-bold text-primary">
                  {new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(placeholderTotalValue)}
                </div>
              </UICard>
